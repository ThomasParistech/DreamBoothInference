...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--save_second_sample_prompt",
        type=str,
        default=None,
        help="The second prompt used to generate sample outputs to save.",
    )
    parser.add_argument(
        ...
        for concept in concepts_list:
            ...
            if with_prior_preservation:
                class_img_path = [(x, concept["class_prompt"])
                                  for x in Path(concept["class_data_dir"]).iterdir() if x.is_file()]
            ...
            if args.save_sample_prompt is not None:
                ...
                with torch.autocast("cuda"), torch.inference_mode():
                    for i in tqdm(range(args.n_save_sample), desc="Generating samples"):
                        ...
                        images[0].save(os.path.join(sample_dir, f"{basename}.png"))

                        if args.save_second_sample_prompt is not None:
                            images = pipeline(
                                args.save_second_sample_prompt,
                                negative_prompt=args.save_sample_negative_prompt,
                                guidance_scale=args.save_guidance_scale,
                                num_inference_steps=args.save_infer_steps,
                                generator=g_cuda
                            ).images
                            images[0].save(os.path.join(sample_dir, f"second_{basename}.png"))
